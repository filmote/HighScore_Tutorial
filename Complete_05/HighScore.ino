...
struct EEPROM_Utils {
  ...
  */
  ...
  void saveSlot(Slot &slot) {
      ...
      EEPROM.update(EEPROM_TOP_START + (EEPROM_ENTRY_SIZE * slot.slotNumber), slot.chars[0]);
      EEPROM.update(EEPROM_TOP_START + (EEPROM_ENTRY_SIZE * slot.slotNumber) + 1, slot.chars[1]);
      EEPROM.update(EEPROM_TOP_START + (EEPROM_ENTRY_SIZE * slot.slotNumber) + 2, slot.chars[2]);
      EEPROM.update(EEPROM_TOP_START + (EEPROM_ENTRY_SIZE * slot.slotNumber), slot.chars[0]);
      EEPROM.put(EEPROM_TOP_START + (EEPROM_ENTRY_SIZE * slot.slotNumber) + 3, slot.score);
